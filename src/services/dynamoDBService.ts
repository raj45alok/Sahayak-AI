...
export const dynamoDBService = {
  ...
  saveUser: async (firebaseUser: any, userType: 'student' | 'teacher', additionalData?: any) => {
    ...
    const userData = {
      ...
      phone: additionalData?.phone || '',
       parentEmail: additionalData?.parentEmail || '',  // ADD THIS
      parentPhone: additionalData?.parentPhone || '',   // ADD THIS
  ...
  getSubmissionsByStudent: async (studentId: string) => {
    const result = await docClient.send(new QueryCommand({
      ...
      KeyConditionExpression: 'SK = :studentId',
      ExpressionAttributeValues: {
        ':studentId': studentId,
      },
    }));
  ...
  gradeSubmission: async (assignmentId: string, studentId: string, grade: number, feedback: string) => {
    await docClient.send(new UpdateCommand({
      ...
      UpdateExpression: 'SET grade = :grade, feedback = :feedback, #status = :status',
