...
const [profileData, setProfileData] = useState<any>(null);
  ...
  try {
    const response = await profileAPI.getStudentProfile(user?.userId || '');
    if (response.success) {
      setProfileData(response.data);
      // Update formData with loaded profile
      setFormData({
        name: response.data.name || user?.name || '',
        email: response.data.email || user?.email || '',
        grade: response.data.grade || user?.grade || '',
        parentEmail: response.data.parentEmail || user?.parentEmail || '',
        parentPhone: response.data.parentPhone || user?.parentPhone || '',
        bio: response.data.bio || 'I love learning new things, especially mathematics!',
        interests: response.data.interests || ['Mathematics', 'Science', 'Reading'],
        learningGoals: response.data.learningGoals || 'Improve in algebra and geometry'
      });
    }
  } catch (error) {
